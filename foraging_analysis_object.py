...
class Foraging_analysis_object():
    ...
    def calculate_questionnaires(self):
        self.oci_df = pg.Parameters_generator.calc_oci(self)
        self.dass_df = pg.Parameters_generator.calc_dass(self)
        self.aaq_df = pg.Parameters_generator.calc_aaq(self)
        return self
    ...
    def calculate_all_paramteres(self):
        ...
        self.all_paramteres_df = p1.join([p2, p3, p4, p5, p6a, p6b, p7, p8, p9, p10a, p10b, p11a, p11b, p13, self.oci_df, self.dass_df, self.aaq_df, self.subject_df], how='inner')
    ...
    def clean_outliers(self):
        ...
        for std in stds_list:
            queried = self.all_paramteres_df.query(std)
            self.add_outliers(std,  self.all_paramteres_df, queried)
            self.all_paramteres_df = queried
        # Save outliers to a file
        pd.DataFrame(self.outliers, index=[0]).transpose().to_csv(self.path + 'results/' + 'outliers' + '.csv', mode='w')
        print("Exported outliers to csv")
        return self
    ...
    def all_parameters_to_csv(self, file_name):
        ...
        self.all_paramteres_df.to_csv(self.path + 'results/' + file_name + '.csv', mode='w')
        print("Exported to csv: " + file_name)

    def add_outliers(self, name, df, queried_df):
        outliers = [s for s in df.index.tolist() if s not in queried_df.index.tolist()]
        for s in outliers:
            self.outliers[s] = name
